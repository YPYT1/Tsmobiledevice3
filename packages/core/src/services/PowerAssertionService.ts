...
export class PowerAssertionService {
  static readonly SERVICE_NAME = 'com.apple.mobile.assertion_agent';
  ...
  async createPowerAssertion(type: string, name: string, timeoutSecs: number, details?: string): Promise<void> {
    const msg: Record<string, any> = {
      CommandKey: 'CommandCreateAssertion',
      AssertionTypeKey: type,
      AssertionNameKey: name,
      AssertionTimeoutKey: timeoutSecs,
    };
    if (details != null) msg.AssertionDetailKey = details;
    await this.sendRecv(msg);
  }
