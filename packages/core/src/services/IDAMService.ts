...
export class IDAMService {
  static readonly SERVICE_NAME = 'com.apple.idamd';
  static readonly RSD_SERVICE_NAME = 'com.apple.idamd.shim.remote';
  ...
  async configurationInquiry(): Promise<Record<string, any>> {
    return this.sendRecv({ 'Configuration Inquiry': true });
  }

  async setIdamConfiguration(value: boolean): Promise<void> {
    await this.sendRecv({ 'Set IDAM Configuration': value });
  }
