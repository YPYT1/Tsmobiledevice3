...
export type DtxPrimitive = null | string | Buffer | number | bigint;
...
function encodeValue(arg: any): Buffer {
  ...
  if (typeof arg === 'bigint') {
    ...
    b.writeUInt32LE(8, 4);
    b.writeBigInt64LE(arg, 8);  // note: pym uses signed
    return b;
  }
...
export function encodeAux(args: any[]): Buffer {
  ...
  // Build body: for each arg, write [PNull key][value]
  const bodyParts: Buffer[] = [];
  for (const arg of args) {
    bodyParts.push(encodePNull());   // key = PNULL
    bodyParts.push(encodeValue(arg)); // value
  }
  const body = Buffer.concat(bodyParts);

  // Header: u32 magic + u32 flags(0) + u64 body_len
...
export function decodeAux(buf: Buffer): DtxPrimitive[] {
  ...
  while (pos < end) {
    // read key (should be PNULL, skip it)
    const keyTag = buf.readUInt32LE(pos); pos += 4;
    if (keyTag !== PNULL_TAG) break; // unexpected
